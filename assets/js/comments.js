...
// hi ryu hi celestial n hi nerds 

// The values in this section are REQUIRED for the widget to work! Keep them in quotes!
const s_stylePath = '/assets/css/comments.css';
const s_formId = '1FAIpQLSf6btKs8Z64zLNt8y7F7zY_LnwR4ZzaROn-faKzQzOsKaJ0vA';
const s_nameId = '775405435';
const s_websiteId = '71835670';
const s_textId = '57291518';
const s_pageId = '260978631';
const s_replyId = '1891061032';
const s_sheetId = '11L1QA7Vzr7qjKDiU3UbaFP2hrt34M-aCZ7JrDnh7DsA';
...
const s_commentsPerPage = 6; // The max amount of comments that can be displayed on one page, any number >= 1 (Replies not counted)
...
const s_textFieldLabel = 'max 500 characters!';
...
const v_mainHtml = `
    <div id="c_inputDiv">
        <fieldset style="padding-top: 13px; padding-bottom:0;"> <!-- what the fuck -->
            <legend>${s_widgetTitle}</legend>
...
const v_formHtml = `
    <div id="c_nameWrapper" class="c-inputWrapper">
        ...
        <input class="c-input c-websiteInput" title="it's optional!" placeholder="${s_websiteFieldLabel}" name="entry.${s_websiteId}" id="entry.${s_websiteId}" type="url" pattern="https://.*">
    </div>

    <div id="c_textWrapper" class="c-inputWrapper">
        <textarea class="c-input c-textInput" placeholder="${s_textFieldLabel}" name="entry.${s_textId}" id="entry.${s_textId}" rows="4" cols="50"  maxlength="${s_maxLength}" required></textarea>
    </div>

    <button id="c_submitButton" name="c_submitButton" type="submit" disabled>${s_submitButtonLabel}</button>
`;
...
if (s_commentsOpen) {
    ...
    const submitBtn = document.getElementById('c_submitButton');

    function validateInputs() {
        const nameValue = nameInput.value.trim();
        const textValue = textInput.value.trim();
        
        // Enable submit button only if both fields have non-whitespace content
        submitBtn.disabled = !(nameValue && textValue);
    }

    // Add input event listeners to both required fields
...
function createComment(data) {
    ...
    if(data.Admin == true) {
        ...
        img.style = 'margin-left: 5px; width:24px; height:24px; image-rendering:pixelated;'
        img.title = "sitemaster"
        top.appendChild(img);
    } 
