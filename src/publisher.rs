...
pub async fn publish(dist_dir: &str) -> Result<(), reqwest::Error> {
    ...
    let site_root = env::var("ROOT").expect("ROOT must be set");

    let client = reqwest::Client::new();

    // let _ = fs::remove_file("site.zip").unwrap();
    let _ = zip_dir(&site_root, dist_dir, "site.zip");
...
fn zip_dir(site_root: &str, src_dir: &str, dst_file: &str) -> zip::result::ZipResult<()> {
    ...
    for entry in WalkDir::new(path) {
        ...
        let full_name = site_root.to_string() + "/" + &name_str; // fucking evil
