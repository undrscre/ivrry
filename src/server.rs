...
pub async fn serve_pages(inp: HashMap<String, String>, env: &Environment<'static>) {
    *PAGES.write().unwrap() = inp;
    ...
    let catchall = warp::path::full()
        .and_then(|full_path: warp::path::FullPath| {
    ...
    task::spawn(async move {
        for res in rx {
            ...
            for path in event.paths {
                ...
                let rel_path: PathBuf = path.strip_prefix(std::env::current_dir().unwrap().join("templates"))
                    .or_else(|_| path.strip_prefix("templates"))
                    .unwrap_or(&path)
                    .to_path_buf();
                
                let filename = rel_path.as_os_str().to_str().unwrap().replace("\\", "/");
                let source = std::fs::read_to_string(&path).unwrap();

                env.remove_template(filename.as_str());
                env.add_template_owned(filename.to_owned(), source).unwrap();

                let result = generate_page(&env, filename.as_str()).await.unwrap();
